...
class TimeLabelMLP(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, out_dim):
        super().__init__()
        self.t_mlp = nn.Sequential(nn.Linear(embedding_dim, hidden_dim),
                                 nn.SiLU(),
                                 nn.Linear(hidden_dim, out_dim))
        self.y_emb = nn.Embedding(num_embeddings=10, embedding_dim=out_dim)
        self.act = nn.SiLU()

    def forward(self, x, t,y,uncond=False):
        t_emb = self.t_mlp(t).unsqueeze(-1).unsqueeze(-1)
        if uncond:
            return self.act(x+t_emb)
        else:
            y_emb = self.y_emb(y).unsqueeze(-1).unsqueeze(-1)
            x = x + t_emb+y_emb
            return self.act(x)
...
class EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, time_embedding_dim):
        ...
        self.time_label_mlp = TimeLabelMLP(embedding_dim=time_embedding_dim, hidden_dim=out_channels, out_dim=out_channels // 2)
        self.conv1 = ResidualDownsample(out_channels // 2, out_channels)

    def forward(self, x, t=None,y=None,uncond=False):
        x_shortcut = self.conv0(x)
        if t is not None:
            x = self.time_label_mlp(x_shortcut, t,y,uncond)
        x = self.conv1(x)
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, time_embedding_dim):
        ...
        self.time_label_mlp = TimeLabelMLP(embedding_dim=time_embedding_dim, hidden_dim=in_channels, out_dim=in_channels // 2)
        self.conv1 = ResidualBottleneck(in_channels // 2, out_channels // 2)

    def forward(self, x, x_shortcut, t=None,y=None,uncond=False):
        x = self.upsample(x)
        x = torch.cat([x, x_shortcut], dim=1)
        x = self.conv0(x)
        if t is not None:
            x = self.time_label_mlp(x, t,y,uncond)
        x = self.conv1(x)
...
class Unet(nn.Module):
    ...
    def __init__(self, timesteps, time_embedding_dim, in_channels=1, out_channels=1, base_dim=64,
                 dim_mults=[2, 4]):
        super().__init__()
